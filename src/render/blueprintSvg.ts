import { pointsToPath as smoothPath } from '../lib/svgPath.ts';
import { computeLevels } from '../lib/graphLevels.ts';
...
function serviceColor(label: string): string {
  ...
  if (/\b(db|database|store|storage|cache|config|data|sql|mongo|redis)\b/.test(l))
    return C.teal;
  return C.blue;
}
...
function bboxOf(nodes: GraphNode[]) {
  ...
  nodes.forEach((n) => {
    const w = n.width || 0,
      h = n.height || 0;
    x0 = Math.min(x0, (n.x || 0) - w / 2);
    y0 = Math.min(y0, (n.y || 0) - h / 2);
    x1 = Math.max(x1, (n.x || 0) + w / 2);
    y1 = Math.max(y1, (n.y || 0) + h / 2);
  });
  return { x0, y0, x1, y1 };
}
...
export function buildBlueprintSvg(
  ...
  const roleOf = (id: string) => resolveRole(roles, id);
  ...
  const title = opts?.title || profile.title;
  ...
  const out: string[] = [];
  out.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" font-family="'Segoe UI','Helvetica Neue',Arial,sans-serif">`,
  );
  out.push(`<defs>
    ...
    <marker id="bpCross" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M1 1L9 9M1 9L9 1" stroke="#8b8698" stroke-width="2"/></marker>
    <marker id="bpCircle" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="8" markerHeight="8" orient="auto"><circle cx="5" cy="5" r="3" fill="#8b8698"/></marker>
  </defs>`);
  out.push(`<rect x="0" y="0" width="${width}" height="${height}" fill="${C.bg}"/>`);

  // Title
  out.push(
    `<text x="40" y="46" font-size="24" font-weight="700" fill="${C.text}">${esc(title)}</text>`,
  );
  if (subtitle)
    out.push(
      `<text x="40" y="72" font-size="13" fill="${C.muted}">${esc(subtitle)}</text>`,
    );

  // Pipeline container + zone headers
  if (pipeline.length) {
    const b = bboxOf(pipeline);
    const p = 22;
    out.push(
      `<rect x="${b.x0 - p}" y="${b.y0 - p}" width="${b.x1 - b.x0 + 2 * p}" height="${b.y1 - b.y0 + 2 * p}" rx="16" fill="#ffffff" stroke="${C.zoneStroke}" stroke-width="1.5"/>`,
    );
    out.push(
      `<text x="${(b.x0 + b.x1) / 2}" y="${b.y0 - p - 12}" text-anchor="middle" font-size="11" font-weight="700" letter-spacing="0.1em" fill="${C.accentLight}">${esc(profile.primaryZone.toUpperCase())}</text>`,
    );
  }
  if (leadIn.length) {
    const b = bboxOf(leadIn);
    out.push(
      `<text x="${(b.x0 + b.x1) / 2}" y="${b.y0 - 18}" text-anchor="middle" font-size="11" font-weight="700" letter-spacing="0.1em" fill="${C.accentLight}">${esc(profile.entryZone.toUpperCase())}</text>`,
    );
  }
  if (outputs.length) {
    const b = bboxOf(outputs);
    out.push(
      `<text x="${(b.x0 + b.x1) / 2}" y="${b.y0 - 18}" text-anchor="middle" font-size="11" font-weight="700" letter-spacing="0.1em" fill="${C.green}">${esc(profile.outputZone.toUpperCase())}</text>`,
    );
  }
  ...
  laid.edges.forEach((e) => {
        ...
        : e.arrow === 'cross'
          ...
          : e.arrow === 'circle'
            ...
            : dashed
              ...
              : 'bpArw';
    out.push(
      `<path d="${d}" fill="none" stroke="${dashed ? C.dash : C.edge}" stroke-width="${e.thick ? 3.5 : dashed ? 1.6 : 1.8}"${dashed ? ' stroke-dasharray="6 5"' : ''} ${marker ? 'marker-end="url(#' + marker + ')"' : ''}/>`,
    );
    ...
    if (e.label && anchor) {
      ...
      out.push(
        `<rect x="${mx - pw / 2}" y="${my - 9}" width="${pw}" height="18" rx="9" fill="#ffffff" stroke="${dashed ? C.dash : C.zoneStroke}" stroke-width="1"/>`,
      );
      out.push(
        `<text x="${mx}" y="${my + 3.5}" text-anchor="middle" font-size="9.5" font-weight="600" fill="${dashed ? C.dash : C.muted}">${esc(text)}</text>`,
      );
    }
  });
  ...
  nodes.forEach((n) => {
    ...
    const fullTitle = lines[0] || n.id;
    const title0 = fullTitle.length > 22 ? fullTitle.slice(0, 21) + '…' : fullTitle;
    const fullSub = lines.slice(1).join(' ');
    const sub = fullSub.length > 29 ? fullSub.slice(0, 28) + '…' : fullSub;
    const isRoot = n.id === rootId;

    if (isRoot) {
      out.push(
        `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="12" fill="${C.dark}" filter="url(#bpSh)"/>`,
      );
      out.push(
        `<text x="${n.x}" y="${(n.y || 0) - 4}" text-anchor="middle" font-size="13" font-weight="700" fill="#ffffff">${esc(title0)}</text>`,
      );
      if (sub)
        out.push(
          `<text x="${n.x}" y="${(n.y || 0) + 15}" text-anchor="middle" font-size="10.5" fill="#c7c6d0">${esc(sub)}</text>`,
        );
      return;
    }
    ...
    out.push(
      `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="11" fill="${fill}" stroke="${stroke}" stroke-width="1.4"${shadow}/>`,
    );

    // Accent bar for services / outputs
    if (role === 'service')
      out.push(
        `<rect x="${x}" y="${y}" width="6" height="${h}" rx="3" fill="${serviceColor(n.label)}"/>`,
      );
    if (role === 'output')
      out.push(
        `<rect x="${x}" y="${y}" width="6" height="${h}" rx="3" fill="${C.green}"/>`,
      );
    ...
    if (role === 'pipeline' && numberOf.has(n.id)) {
      ...
      out.push(
        `<text x="${x + 22}" y="${y - 7}" text-anchor="middle" font-size="12" font-weight="700" fill="#fff">${numberOf.get(n.id)}</text>`,
      );
    }

    out.push(
      `<text x="${n.x}" y="${(n.y || 0) - 4}" text-anchor="middle" font-size="12.5" font-weight="700" fill="${C.text}">${esc(title0)}</text>`,
    );
    if (sub)
      out.push(
        `<text x="${n.x}" y="${(n.y || 0) + 15}" text-anchor="middle" font-size="10" fill="${C.muted}">${esc(sub)}</text>`,
      );
  });

  // Footer legend
  out.push(
    `<text x="40" y="${height - 22}" font-size="11" fill="#a0a0ab">Solid = main data flow &#183; Dashed = service / AI calls</text>`,
  );
