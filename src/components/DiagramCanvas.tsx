...
function NodeIcon({ type, color }: { type: NodeType; color: string }) {
  ...
  switch (type) {
    case 'user':
      return (
        <g className="icon-bounce">
          <circle cx="8" cy="5.5" r="2.5" stroke={color} strokeWidth="1.4" fill="none" />
          <path d="M2 15 Q2 10 8 10 Q14 10 14 15" {...s} />
        </g>
      );
    case 'client':
      return (
        <g className="icon-bounce">
          <rect x="2" y="4" width="12" height="8" rx="1.5" {...s} />
          <line x1="1" y1="12" x2="15" y2="12" stroke={color} strokeWidth="1.4" />
          <line x1="6" y1="12" x2="10" y2="14" stroke={color} strokeWidth="1.4" />
        </g>
      );
    case 'gateway':
      return (
        <g className="icon-pulse">
          <path d="M8 1.5L14.5 8L8 14.5L1.5 8Z" {...s} />
          <line x1="8" y1="5" x2="8" y2="11" stroke={color} strokeWidth="1.4" />
          <line x1="5" y1="8" x2="11" y2="8" stroke={color} strokeWidth="1.4" />
        </g>
      );
    case 'loadbalancer':
      return (
        <g className="icon-spin">
          <path d="M8 3 v3 M4 12 h8 M8 6 L4 12 M8 6 L12 12" {...s} />
    ...
    case 'service':
      return (
        <g className="icon-spin">
          ...
          <path
            d="M8 1v2 M8 13v2 M1 8h2 M13 8h2 M3.1 3.1l1.4 1.4 M11.5 11.5l1.4 1.4 M3.1 12.9l1.4-1.4 M11.5 4.5l1.4-1.4"
            {...s}
          />
        </g>
      );
    case 'database':
      return (
        <g className="icon-pulse">
          <ellipse cx="8" cy="4.5" rx="5" ry="1.8" {...s} />
          <path d="M3 4.5v7 Q3 14 8 14 Q13 14 13 11.5v-7" {...s} />
          <path d="M3 8 Q3 10 8 10 Q13 10 13 8" {...s} />
        </g>
      );
    case 'cache':
      return (
        <g className="icon-pulse">
          <path
            d="M8 1.5 L11.5 6.5 H9.5 V9.5 H11.5 L8 14.5 L4.5 9.5 H6.5 V6.5 H4.5 Z"
            stroke={color}
            strokeWidth="1.4"
            fill={color}
            fillOpacity="0.25"
    ...
    case 'external':
      return (
        <g className="icon-spin">
          ...
          <path d="M8 2 Q11 8 8 14 Q5 8 8 2" {...s} />
        </g>
      );
  }
}
...
export function DiagramCanvas({ graph, theme = 'dark', reducedMotion = false }: Props) {
            ...
            return (
              <g key={grp.id} id={`group-${grp.id}`}>
                <rect
                  ...
                  fill={
                    grp.color ??
                    (isLight ? 'rgba(241,245,249,0.5)' : 'rgba(100,116,139,0.08)')
                  }
                ...
                <text
                  x={gx + 12}
                  y={gy + 14}
                  fill={isLight ? 'rgba(71,85,105,0.7)' : 'rgba(255,255,255,0.4)'}
                  fontSize="10"
              ...
              <g key={node.id} transform={`translate(${rx},${ry})`}>
                ...
                >
                  ...
                  <rect
                    ...
                    stroke={node.color ? 'rgba(255,255,255,0.2)' : st.border}
                    strokeWidth="1.5"
                  />
